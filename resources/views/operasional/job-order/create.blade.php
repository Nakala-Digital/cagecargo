...
<div class="max-w-4xl">
    <div class="mb-6">
        <h2 class="text-2xl font-bold text-navy">Buat Job Order</h2>
        <p class="text-gray-500">Form job order baru</p>
    </div>
    <div class="bg-white rounded-xl shadow-sm p-6">
        <form method="POST" action="{{ route('operasional.job-order.store') }}">
            @csrf
            <div class="grid grid-cols-3 gap-4">
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="nomor_jo" value="{{ old('nomor_jo') }}" placeholder="Otomatis jika dikosongkan" class="w-full px-4 py-2.5 border rounded-lg focus:ring-2 focus:ring-teal focus:border-teal @error('nomor_jo') border-red-500 @enderror">
                ...
                <div class="mb-4">
                    ...
                    <select name="consignee_id" class="w-full px-4 py-2.5 border rounded-lg focus:ring-2 focus:ring-teal focus:border-teal">
                        ...
                        @foreach($customers as $c)
                        <option value="{{ $c->id }}" {{ old('consignee_id') == $c->id ? 'selected' : '' }}>{{ $c->nama }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="mb-4">
                    ...
                    <select name="shipper_id" class="w-full px-4 py-2.5 border rounded-lg focus:ring-2 focus:ring-teal focus:border-teal">
                        ...
                        @foreach($customers as $c)
                        <option value="{{ $c->id }}" {{ old('shipper_id') == $c->id ? 'selected' : '' }}>{{ $c->nama }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="mb-4">
                    ...
                    <select name="notify_party_id" class="w-full px-4 py-2.5 border rounded-lg focus:ring-2 focus:ring-teal focus:border-teal">
                        ...
                        @foreach($customers as $c)
                        <option value="{{ $c->id }}" {{ old('notify_party_id') == $c->id ? 'selected' : '' }}>{{ $c->nama }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="col-span-3 mb-4">
                    ...
                    <input type="text" name="jenis_barang" value="{{ old('jenis_barang') }}" class="w-full px-4 py-2.5 border rounded-lg focus:ring-2 focus:ring-teal focus:border-teal">
                </div>
                <div class="mb-4">
                    <label class="block text-sm font-medium text-gray-700 mb-1">Berat (kg)</label>
                    <input type="number" name="berat" value="{{ old('berat') }}" class="w-full px-4 py-2.5 border rounded-lg focus:ring-2 focus:ring-teal focus:border-teal">
                </div>
                <div class="mb-4">
                    <label class="block text-sm font-medium text-gray-700 mb-1">Volume (m3)</label>
                    <input type="number" step="0.01" name="volume" value="{{ old('volume') }}" class="w-full px-4 py-2.5 border rounded-lg focus:ring-2 focus:ring-teal focus:border-teal">
                </div>
                <div class="mb-4">
                    <label class="block text-sm font-medium text-gray-700 mb-1">Jumlah Container</label>
                    <input type="number" name="jumlah_container" value="{{ old('jumlah_container') }}" class="w-full px-4 py-2.5 border rounded-lg focus:ring-2 focus:ring-teal focus:border-teal">
                </div>
                <div class="col-span-3 grid grid-cols-3 gap-4">
                    <div class="mb-4">
                        <label class="block text-sm font-medium text-gray-700 mb-1">Asal</label>
                        <input type="text" name="asal" value="{{ old('asal') }}" class="w-full px-4 py-2.5 border rounded-lg focus:ring-2 focus:ring-teal focus:border-teal">
                    </div>
                    <div class="mb-4">
                        <label class="block text-sm font-medium text-gray-700 mb-1">Tujuan</label>
                        <input type="text" name="tujuan" value="{{ old('tujuan') }}" class="w-full px-4 py-2.5 border rounded-lg focus:ring-2 focus:ring-teal focus:border-teal">
                    </div>
                    <div class="mb-4">
                        <label class="block text-sm font-medium text-gray-700 mb-1">HS Code</label>
                        <input type="text" name="hs_code" value="{{ old('hs_code') }}" class="w-full px-4 py-2.5 border rounded-lg focus:ring-2 focus:ring-teal focus:border-teal">
                    </div>
                    <div class="mb-4">
                        <label class="block text-sm font-medium text-gray-700 mb-1">Pelabuhan Asal</label>
                        <input type="text" name="pelabuhan_asal" value="{{ old('pelabuhan_asal') }}" class="w-full px-4 py-2.5 border rounded-lg focus:ring-2 focus:ring-teal focus:border-teal">
                    </div>
                    <div class="mb-4">
                        <label class="block text-sm font-medium text-gray-700 mb-1">Pelabuhan Tujuan</label>
                        <input type="text" name="pelabuhan_tujuan" value="{{ old('pelabuhan_tujuan') }}" class="w-full px-4 py-2.5 border rounded-lg focus:ring-2 focus:ring-teal focus:border-teal">
                    </div>
                    <div class="mb-4">
                        <label class="block text-sm font-medium text-gray-700 mb-1">Incoterms</label>
                        <input type="text" name="incoterms" value="{{ old('incoterms') }}" class="w-full px-4 py-2.5 border rounded-lg focus:ring-2 focus:ring-teal focus:border-teal">
                    </div>
                    <div class="mb-4">
                        <label class="block text-sm font-medium text-gray-700 mb-1">Negara Asal</label>
                        <input type="text" name="negara_asal" value="{{ old('negara_asal') }}" class="w-full px-4 py-2.5 border rounded-lg focus:ring-2 focus:ring-teal focus:border-teal">
                    </div>
                    <div class="mb-4">
                        <label class="block text-sm font-medium text-gray-700 mb-1">Negara Tujuan</label>
                        <input type="text" name="negara_tujuan" value="{{ old('negara_tujuan') }}" class="w-full px-4 py-2.5 border rounded-lg focus:ring-2 focus:ring-teal focus:border-teal">
                    </div>
                    <div class="mb-4">
                        <label class="block text-sm font-medium text-gray-700 mb-1">Vessel / Voyage</label>
                        <div class="flex space-x-2">
                            <input type="text" name="vessel" value="{{ old('vessel') }}" placeholder="Vessel" class="w-1/2 px-4 py-2.5 border rounded-lg focus:ring-2 focus:ring-teal focus:border-teal">
                            <input type="text" name="voyage" value="{{ old('voyage') }}" placeholder="Voyage" class="w-1/2 px-4 py-2.5 border rounded-lg focus:ring-2 focus:ring-teal focus:border-teal">
                        </div>
                    </div>
                    <div class="mb-4">
                        <label class="block text-sm font-medium text-gray-700 mb-1">ETD</label>
                        <input type="date" name="etd" value="{{ old('etd') }}" class="w-full px-4 py-2.5 border rounded-lg focus:ring-2 focus:ring-teal focus:border-teal">
                    </div>
                    <div class="mb-4">
                        <label class="block text-sm font-medium text-gray-700 mb-1">ETA</label>
                        <input type="date" name="eta" value="{{ old('eta') }}" class="w-full px-4 py-2.5 border rounded-lg focus:ring-2 focus:ring-teal focus:border-teal">
                    </div>
                </div>
                ...
                <div class="mb-4">
                    ...
                    <select name="armada_id" class="w-full px-4 py-2.5 border rounded-lg focus:ring-2 focus:ring-teal focus:border-teal">
                        ...
                        <option value="{{ $a->id }}" {{ old('armada_id') == $a->id ? 'selected' : '' }}>{{ $a->nomor_polisi }}</option>
                        @endforeach
                    </select>
                </div>
                <div class="mb-4">
                    ...
                    <select name="driver_id" class="w-full px-4 py-2.5 border rounded-lg focus:ring-2 focus:ring-teal focus:border-teal">
                        ...
                        <option value="{{ $d->id }}" {{ old('driver_id') == $d->id ? 'selected' : '' }}>{{ $d->nama }}</option>
                        @endforeach
                    </select>
                </div>
                ...
                <div class="col-span-3 mb-4">
                    <div class="border rounded-lg p-4 max-h-48 overflow-y-auto">
                        ...
                        <label class="flex items-center space-x-3 py-1.5 hover:bg-gray-50 px-2 rounded">
                            <input type="checkbox" name="container_ids[]" value="{{ $container->id }}" {{ in_array($container->id, old('container_ids', [])) ? 'checked' : '' }} class="text-teal focus:ring-teal rounded">
                ...
                <div class="col-span-3 mb-4">
                    <textarea name="catatan" rows="3" class="w-full px-4 py-2.5 border rounded-lg focus:ring-2 focus:ring-teal focus:border-teal">{{ old('catatan') }}</textarea>
                </div>
            </div>
