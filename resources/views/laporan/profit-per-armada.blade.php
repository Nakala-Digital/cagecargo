@extends('layouts.app')
@section('title', 'Profit per Armada')
@section('content')
<div class="space-y-6">
    <div>
        <h2 class="text-2xl font-bold text-navy">Profit per Armada</h2>
        <p class="text-gray-500">Analisa profitabilitas per unit armada</p>
    </div>
    ...
    <form method="GET" class="flex gap-4 items-end bg-white rounded-xl shadow-sm p-4">
        ...
        <a href="{{ route('laporan.profit-per-armada') }}" class="px-4 py-2 border border-gray-300 rounded-lg text-sm text-gray-700 hover:bg-gray-50">Reset</a>
    </form>

    <div class="bg-white rounded-xl shadow-sm p-6">
        <h3 class="text-sm font-semibold text-navy mb-4">Top 10 Armada by Profit</h3>
        ...
        <table class="w-full">
            ...
            <tbody class="divide-y divide-gray-100">
                ...
                <tr class="hover:bg-gray-50">
                    <td class="px-6 py-4 text-sm font-medium">{{ $row->a->nomor_polisi }} ({{ $row->a->jenis_kendaraan }})</td>
...
@push('scripts')
<script>
const topArmadas = @json($armadas->take(10)->values());
const labels = topArmadas.map(r => (r.a.nomor_polisi || '').length > 12 ? (r.a.nomor_polisi || '').substring(0, 12) + '..' : (r.a.nomor_polisi || ''));
const profits = topArmadas.map(r => r.profit);
